...
"""
  ...
  Note: As the task was to "build an HTTP proxy" I used two libraries to "HTTPServer" and "BaseHTTPRequestHandler" to handle the requests (http server)
        and "requests" as client to perform the upstream http requests. Otherwise the python proxy library cpould be used https://pypi.org/project/proxy.py/
...
class handler(BaseHTTPRequestHandler):
    def forward(self, type):
        ...
        postmanEcho = "http://postman-echo.com/"
        myEcho = "http://localhost:88/"
        fwUrl = myEcho + type.lower()
        newHeaders = {}
        ...
        if (type.lower() == 'post'):
            ...
            logging.debug("New Headers (after appending the intended one): %s", newHeaders)
